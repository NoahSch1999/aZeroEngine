...
bool aZero::D3D12::Shader::CompileFromFile(IDxcCompiler3& Compiler, const std::string& ShaderFileName)
{
	this->Reset();

	const std::string FilePath = Helper::GetProjectDirectory() + SHADER_SOURCE_RELATIVE_PATH + ShaderFileName;
	...
	std::string OutputPath(ProjectPath + "/ShaderOutput/" + std::string(ShaderPath.begin(), ShaderPath.end()));
