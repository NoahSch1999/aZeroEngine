...
namespace aZero
{
	namespace Rendering
	{
		...
		{
			m_MeshCullCS_NEW.Compile(m_diCompiler, Pipeline::GetShaderDirectoryPath() + "MeshCull.cs.hlsl");
			m_MeshCullPass_NEW.CompileComputePass(m_diDevice, m_MeshCullCS_NEW);

			m_MeshletDrawAS_NEW.Compile(m_diCompiler, Pipeline::GetShaderDirectoryPath() + "MeshletDraw.as.hlsl");
			m_MeshletDrawMS_NEW.Compile(m_diCompiler, Pipeline::GetShaderDirectoryPath() + "MeshletDraw.ms.hlsl");
			m_MeshletDrawPS_NEW.Compile(m_diCompiler, Pipeline::GetShaderDirectoryPath() + "Default_Phong.ps.hlsl");
		...
		void Renderer::RecordGPUDrivenRenderPipeline_NEW(Rendering::RenderTarget& renderTarget, Rendering::DepthStencilTarget& depthStencilTarget, const Rendering::GPUProxy::Camera& camera, uint32_t numStaticMeshes)
		{
...

			{
				...
				auto cameraBuffer = m_MeshCullPass_NEW.GetBufferBinding("CameraBuffer");
...

			{
				...
				auto meshletCullConstants = m_MeshletDrawPass_NEW.GetConstantBinding("MeshletCull_CONSTANT");
				auto meshInstancesBuffer = m_MeshletDrawPass_NEW.GetBufferBinding("MeshInstances");
				auto cameraBuffer = m_MeshletDrawPass_NEW.GetBufferBinding("CameraBuffer");
