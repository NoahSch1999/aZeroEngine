...
)
{
	...
	for (int i = 0; i < meshlet_count; i++)
	{
		...
		newMeshlet.PrimitiveOffset = meshlet.triangle_offset / 3 ;
...
aZero::Asset::Mesh::Mesh(const FBX::FBX_Mesh& mesh)
{
	for (const FBX::FBX_Submesh& submesh : mesh.Submeshes)
	{
		...
		newSubmesh.VertexOffset = m_VertexData.Vertices.size();
		newSubmesh.PrimitiveOffset = m_VertexData.Primitives.size();
		newSubmesh.IndexOffset = m_VertexData.Indices.size();

		std::vector<Vertex> vertices(submesh.Vertices);
		std::vector<Index> indices(submesh.Indices);
		std::vector<Meshlet> meshlets;
		std::vector<uint32_t> primitives;
		Meshletize(vertices, indices, meshlets, primitives);
	...
	{
		m_VertexData.Name = mesh.Submeshes[0].Name;
	}
}
...
)
{
	...
	std::vector<aZero::Asset::Index> newInd;
...
std::vector<aZero::Asset::MeshletMeshData> LoadFBX(const std::string& path)
{
	...
	const aiScene* const scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace | aiProcess_JoinIdenticalVertices);
