...
namespace aZero
{
	namespace D3D12
	{
		...
		private:
			...
			std::shared_ptr<D3D12::GPUBuffer> m_Buffer;
		...
		public:
			...
			void Write(ID3D12GraphicsCommandListX* CmdList, D3D12::LinearFrameAllocator& Allocator, void* Data, uint32_t Offset, uint32_t NumBytes)
			{
				...
				Allocator.AddAllocation(CmdList, Data, m_Buffer.get(), Offset, NumBytes);
			}
			...
			std::weak_ptr<D3D12::GPUBuffer> GetBuffer() const { return m_Buffer; }
		};
	}
}