...
bool aZero::Scene::Scene::Save(const std::string& filePath)
{
	return true;
}

bool aZero::Scene::Scene::Load(const std::string& filePath)
{
	return true;
}

void aZero::Scene::Scene::AddRigidbody(const ECS::Entity& entity, ECS::RigidbodyComponent* rb)
{
...
void aZero::Scene::Scene::RemoveRigidbody(ECS::RigidbodyComponent* rb)
{
	if (!m_PhysicsWorld.get())
		return;
...
void aZero::Scene::Scene::AddRigidbody(const ECS::Entity& entity, aZero::Physics::Body& body, JPH::BodyCreationSettings& tempBodySettings)
{
	if (!m_PhysicsWorld.get())
		return;
...
void aZero::Scene::Scene::RemoveRigidbody(const aZero::Physics::Body& body)
{
	if (!m_PhysicsWorld.get())
		return;
...
void aZero::Scene::Scene::AddDebugDrawArguments(Rendering::WireframeRenderer& wireframeRenderer, bool showColliders, bool showMeshBounds)
{
	...
	for (auto& [name, entity] : m_Entities)
	{
					...
					for (auto& collider : colliders)
					{
						...
						if (lock->Succeeded())
						{
							...
							if (body->GetShape()->GetSubType() == JPH::EShapeSubType::Box)
							{
								...
								wireframeRenderer.AddShape(Rendering::WireframeShape::OBB(DXM::Vector3(1, 0, 0), Math::Convert(body->GetPosition()), Math::Convert(body->GetRotation()), Math::Convert(boxShape->GetHalfExtent())));
							}
		...
		if (showMeshBounds)
		{
			...
			if (tfComp && smComp)
			{
				...
				wireframeRenderer.AddShape(Rendering::WireframeShape::Sphere(DXM::Vector3(0, 0, 1), DXM::Vector3::Transform(meshData.Bounds.Center, tfComp->GetTransform()), meshData.Bounds.Radius, 10));
			...
			if (rbComp)
			{
				...
				if (lock->Succeeded())
				{
					...
					if (tfComp)
					{
						...
						this->MarkRenderStateDirty(entity, aZero::Scene::Scene::ComponentFlag());
					}
				}
			}
		}
